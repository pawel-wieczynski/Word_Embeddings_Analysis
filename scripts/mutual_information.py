...
class MutualInformation:
    def __init__(self, tokens: list[str], alpha: float = None) -> None:
        self.tokens: list[str] = tokens
        self.tokens_length = len(self.tokens)
        self.tokens_counts = self.count_tokens()
        self.vocabulary_size = len(self.tokens_counts)
        self.alpha = alpha
    ...
    def get_marginal_probability(self, word: str) -> float:
        """
        Lindstone smoothed marginal probability:
            (count(word) + alpha) / (N + alpha * m)
        """
        if self.alpha is None:
            return self.tokens_counts[word] / self.tokens_length
        else:
            return (self.tokens_counts[word] + self.alpha) / (self.tokens_length + self.alpha * self.vocabulary_size)
    ...
    def get_joint_probability(self, word_x: str, word_y: str, joint_counts) -> float:
        ...
        if self.alpha is None:
            return joint_counts[(word_x, word_y)] / number_of_pairs
        else:
            m = len(joint_counts)
            return (joint_counts[(word_x, word_y)] + self.alpha) / (number_of_pairs + self.alpha * m)
    ...
    def calculate_mutual_information(self, lag: int):
        ...
        for (x, y), count_xy in joint_counts.items():
            p_xy = self.get_joint_probability(word_x = x, word_y = y, joint_counts = joint_counts)
